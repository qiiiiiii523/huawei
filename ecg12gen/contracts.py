...
class JointAnchorSample:
    """Cross-time context plus same-record/window target-time I anchor."""
    ...
    def validate(self) -> None:
        ...
        if self.context_ecg.shape != (expected, WINDOW_SAMPLES):
            raise ContractError("context shape does not match actual context channels")
        if self.task_id == "task1" and self.context_source_type != "watch_ecg":
            raise ContractError("task1 context must be watch_ecg")
        if self.task_id == "task2" and self.context_source_type not in {"ecg_machine_d6", "body_scale_d6"}:
            raise ContractError("task2 context must be d6")
        if self.anchor_i_ecg.shape != (1, WINDOW_SAMPLES) or self.Y_12lead.shape != (12, WINDOW_SAMPLES):
            raise ContractError("anchor/target shapes are invalid")
        if self.anchor_source_type != "ecg_machine_i":
            raise ContractError("anchor must be ecg_machine_i")
        if self.anchor_lead_mask.shape != (12,) or not (self.anchor_lead_mask[0] and self.anchor_lead_mask.sum() == 1):
            raise ContractError("only I is observed at target time")
        if self.context_target_relation != "same_subject_cross_time" or self.anchor_target_relation != "same_record_same_window":
            raise ContractError("wrong context/anchor relation labels")
        if self.context_target_sync or not self.anchor_target_sync or not self.pointwise_loss_allowed or not self.anchor_available_at_test:
            raise ContractError("wrong sync/supervision flags")
        required = {"anchor_construction", "anchor_target_record_id", "anchor_window_id"}
        if not required.issubset(self.meta) or self.meta["anchor_target_record_id"] != self.target_record_id or self.meta["anchor_window_id"] != self.window_id:
            raise ContractError("anchor provenance must be same target record/window")
...
class JointAnchorInferenceInput:
    ...
    def validate(self) -> None:
        ...
        if self.task_id not in {"task1", "task2"} or self.context_ecg.shape != (expected, WINDOW_SAMPLES) or self.anchor_i_ecg.shape != (1, WINDOW_SAMPLES):
            raise ContractError("explicit context and machine-I anchor are required")


def prepare_joint_anchor_inference(context_ecg: np.ndarray, *, context_source_type: str,
                                   task_id: str, anchor_i_ecg: np.ndarray | None,
                                   context_channel_indices: tuple[int, ...] | None = None) -> JointAnchorInferenceInput:
    if anchor_i_ecg is None:
        raise ContractError("Joint-anchor inference requires an explicit machine-I anchor")
    item = JointAnchorInferenceInput(np.asarray(context_ecg), context_source_type,
                                     np.asarray(anchor_i_ecg), task_id, context_channel_indices)
    item.validate()
    return item
