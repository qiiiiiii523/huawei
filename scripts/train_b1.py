...
def main():
    p=argparse.ArgumentParser(); p.add_argument("--config",default=str(ROOT/"configs/common.yaml")); p.add_argument("--task-id",choices=("task1","task2"),required=True); p.add_argument("--stage",choices=("P0_anchor_only","P1-C3"),required=True); p.add_argument("--fusion-mode",choices=("none","film_gated_residual"),default="none"); p.add_argument("--p0-checkpoint"); p.add_argument("--context-source-type",choices=("watch_ecg","ecg_machine_d6","body_scale_d6")); p.add_argument("--body-scale-variant",choices=("A_raw_window","B_detrend_0p2Hz_then_window"),default="A_raw_window"); p.add_argument("--variant",choices=("base","wide","dilated","core7","softcore","attention"),default="base"); p.add_argument("--baseline-weight",type=float,default=.10); p.add_argument("--output-dir",required=True); p.add_argument("--device",default="cuda"); p.add_argument("--epochs",type=int,default=60); p.add_argument("--batch-size",type=int,default=8); p.add_argument("--dropout",type=float,default=.10); p.add_argument("--lr",type=float,default=1e-4); p.add_argument("--weight-decay",type=float,default=5e-4); p.add_argument("--seed",type=int,default=42); a=p.parse_args()
