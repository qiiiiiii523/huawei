"""Non-training B2 P0/C1/C2/C3, leakage, and task2 dual-context smoke test."""
...
def main() -> None:
    torch.manual_seed(42); p0 = B2JointAnchorPatchTransformer(B2ModelConfig(fusion_mode="none")); c2 = B2JointAnchorPatchTransformer(B2ModelConfig(fusion_mode="gated_residual")); c3 = B2JointAnchorPatchTransformer(B2ModelConfig(fusion_mode="film_gated_residual")); c2.load_state_dict(p0.state_dict()); c3.load_state_dict(p0.state_dict()); p0.eval(); c2.eval(); c3.eval()
    batch = _batch(); raw_p0, raw_c3 = _forward(p0,batch,"task1"), _forward(c3,batch,"task1")
    decoded = []
    ...
    assert raw_p0.shape == (2,12,5000) and torch.allclose(raw_p0, raw_c3, atol=1e-6)
    assert abs(float(torch.sigmoid(c3.gate.bias).mean().detach()) - .05) < .002
    assert _forward(c2,batch,"task2").shape == (2,12,5000) and _forward(c3,batch,"task2").shape == (2,12,5000)
    assert architecture_metadata(p0.config) == architecture_metadata(c3.config)
    ...
    assert torch.isfinite(joint_anchor_sync_loss(raw_c3,batch["target_model"],batch["anchor_model"],d12_scale_uV=scale))
    changed=batch["target_model"].clone(); changed[:,:1]+=100; assert not torch.allclose(joint_anchor_sync_loss(raw_c3,changed,batch["anchor_model"],d12_scale_uV=scale), joint_anchor_sync_loss(raw_c3,batch["target_model"],batch["anchor_model"],d12_scale_uV=scale))
    ...
    try: fit_b2(c3,machine,machine,pre2.scale_uV_by_source["d12"],"task2",ROOT/"tmp_should_not_exist",stage=P1_STAGE); raise AssertionError("P1 without P0 accepted")
    ...
    print(f"PASS: B2 P0/C1/C2/C3; strict={len(strict)}; T2 machine/body={len(machine)}/{len(body)}; dual train/val={counts_train['both_rows']}/{counts_val['both_rows']}")
